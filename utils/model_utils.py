...
from FLAlgorithms.trainmodel.models import ResNetMSPA

from torch.utils.data import DataLoader
from FLAlgorithms.trainmodel.generator import Generator
from utils.model_config import *
METRICS = ['glob_acc', 'per_acc', 'glob_loss', 'per_loss', 'user_train_time', 'server_agg_time','train_loss',
           'fairness_score','rare_f1','client_acc_std','wasserstein_dist']
...
def get_data_dir(dataset):
    if 'EMnist' in dataset:
    ...
    elif 'sentiment140' in dataset.lower():
        dataset_=dataset.replace('alpha', '').replace('ratio', '').split('-')
        alpha, ratio=dataset_[1], dataset_[2]
        path_prefix = os.path.join('data', 'Sentiment140', 'u20c2-alpha{}-ratio{}'.format(alpha, ratio))
        train_data_dir = os.path.join(path_prefix, 'train')
        test_data_dir = os.path.join(path_prefix, 'test')
        proxy_data_dir = 'data/proxy_data/sentiment140/'
        ...
        alpha, ratio=dataset_[1], dataset_[2]
        #path_prefix=os.path.join('data', 'Mnist', 'u20alpha{}min10ratio{}'.format(alpha, ratio))
        path_prefix=os.path.join('data', 'Cifar', 'u20c10-alpha{}-ratio{}'.format(alpha, ratio))
        train_data_dir=os.path.join(path_prefix, 'train')
        test_data_dir=os.path.join(path_prefix, 'test')
        proxy_data_dir = 'data/proxy_data/cifar-n10/'
    elif 'cifa100' in dataset.lower():
        dataset_=dataset.lower().replace('alpha', '').replace('ratio', '').split('-')
        alpha, ratio=dataset_[1], dataset_[2]
        #path_prefix=os.path.join('data', 'Mnist', 'u20alpha{}min10ratio{}'.format(alpha, ratio))
        path_prefix=os.path.join('data', 'Cifa100', 'u20c100-alpha{}-ratio{}'.format(alpha, ratio))
        train_data_dir=os.path.join(path_prefix, 'train')
        test_data_dir=os.path.join(path_prefix, 'test')
        proxy_data_dir = 'data/proxy_data/cifa100-n10/'
    elif 'retina' in dataset.lower():
        dataset_=dataset.lower().replace('alpha', '').replace('ratio', '').split('-')
        alpha, ratio=dataset_[1], dataset_[2]
        #path_prefix=os.path.join('data', 'Mnist', 'u20alpha{}min10ratio{}'.format(alpha, ratio))
        path_prefix=os.path.join('data', 'Retina', 'u5c2-alpha{}-ratio{}'.format(alpha, ratio))
        train_data_dir=os.path.join(path_prefix, 'train')
        test_data_dir=os.path.join(path_prefix, 'test')
        proxy_data_dir = 'data/proxy_data/retina-n5/'
    elif 'covid' in dataset.lower():
        dataset_=dataset.lower().replace('alpha', '').replace('ratio', '').split('-')
        alpha, ratio=dataset_[1], dataset_[2]
        #path_prefix=os.path.join('data', 'Mnist', 'u20alpha{}min10ratio{}'.format(alpha, ratio))
        path_prefix=os.path.join('data', 'COVID', 'u12c3-alpha{}-ratio{}'.format(alpha, ratio))
        train_data_dir=os.path.join(path_prefix, 'train')
        test_data_dir=os.path.join(path_prefix, 'test')
        proxy_data_dir = 'data/proxy_data/retina-n5/'
    else:
...
def aggregate_data_(clients, dataset, dataset_name, batch_size):
    ...
    for i in range(len(dataset)):
        ...
        if 'sentiment140' in dataset_name.lower():
            # 针对使用BERT的场景，接收三个返回值
            input_ids, attention_mask, y = convert_data( # [!code focus]
                user_data['x'],
                user_data['y'],
            ...
            combined += [
                (
                    ...
                    label
                ) for ids, mask, label in zip(input_ids, attention_mask, y)
            ] # [!code focus]
        else:
            # 常规数据的处理（其他数据集仍返回X,y）
            X, y = convert_data( # [!code focus]
                user_data['x'],
                user_data['y'],
                dataset=dataset_name
            )
            combined += [(x, y) for x, y in zip(X, y)] # [!code focus]
...
def convert_data(X, y, dataset=''):
    if 'sentiment140' in dataset.lower():
        # 保持BERT编码结构
...
def read_user_data(index, data, dataset='', count_labels=False):
    """支持文本数据的修改版本"""
    # data包含：clients, groups, train_data, test_data, proxy_data(可选)
    id = data[0][index]
    ...
    if 'sentiment140' in dataset.lower():
        # 获取文本数据的三个组成部分
        input_ids_train, attention_mask_train, y_train = convert_data(train_data['x'], train_data['y'], dataset=dataset)
        input_ids_test, attention_mask_test, y_test = convert_data(test_data['x'], test_data['y'], dataset=dataset)

        # 修改点2：构建字典格式的数据样本
        train_data = [
            {
                'input_ids': input_ids_train[i],
                'attention_mask': attention_mask_train[i],
                'labels': y_train[i]
            } for i in range(len(y_train))
        ]
        test_data = [
            {
                'input_ids': input_ids_test[i],
                'attention_mask': attention_mask_test[i],
                'labels': y_test[i]
    ...
    if count_labels:
        ...
        if 'sentiment140' in dataset.lower():
            ...
            unique_y, counts = torch.unique(torch.tensor([y for _, y in train_data]), return_counts=True)
...
def get_dataset_name(dataset):
    ...
    elif 'cifa100' in dataset:
        passed_dataset='cifa100'
    elif 'retina' in dataset:
        passed_dataset='retina'
    elif 'covid' in dataset:
        passed_dataset='covid'
    elif 'sentiment140' in dataset:
        passed_dataset='sentiment140'
    else:
        raise ValueError('Unsupported dataset {}'.format(dataset))
    return passed_dataset


# 定义一个工厂函数，根据数据集名称返回相应的网络模型实例
def get_network_model(dataset, model_type,numclass):
    if model_type == 'cnn':
        return Net(dataset=dataset, model='cnn').to(device)
    elif model_type == 'resnet':
        return ResNetMSPA(num_classes=numclass, in_channels=3).to(device)
    else:
...
def create_model(model_type, dataset, algorithm, numclass):

    passed_dataset = get_dataset_name(dataset)
    model = get_network_model(passed_dataset, model_type, numclass)
    return model
