...
def devide_train_data(data, n_sample, SRC_CLASSES, NUM_USERS, min_sample, alpha=0.1, sampling_ratio=0.99):
    min_sample = 800#len(SRC_CLASSES) * min_sample
    ...
    while min_size < min_sample:
        ...
        max_samples_per_user = sampling_ratio * n_sample / (NUM_USERS*0.2*2)
...
def main():
    ...
    parser.add_argument("--min_sample", type=int, default=800, help="Min number of samples per user.")
    parser.add_argument("--sampling_ratio", type=float, default=0.99, help="Ratio for sampling training samples.")
...
#         return [(user_id + j) % SRC_N_CLASS for j in range(NUM_LABELS)]
#
# def devide_train_data(data, n_sample, SRC_CLASSES, NUM_USERS, min_sample, alpha=0.5, sampling_ratio=0.99):
#     min_sample = 1000#len(SRC_CLASSES) * min_sample
#     min_size = 0 # track minimal samples per user
#     total_sampled = 0
#     ###### Determine Sampling #######
#     while min_size < min_sample or total_sampled < n_sample * sampling_ratio :
#         print("Try to find valid data separation")
#         idx_batch=[{} for _ in range(NUM_USERS)]
#         samples_per_user = [0 for _ in range(NUM_USERS)]
#         max_samples_per_user = sampling_ratio * n_sample / (NUM_USERS*0.2*2)
...
#             np.random.shuffle(idx_l)
#             if sampling_ratio < 1:
#                 samples_for_l = int(sampling_ratio * len(data[l]))
#                 idx_l = idx_l[:samples_for_l]
#                 print(l, len(data[l]), len(idx_l))
#             # dirichlet sampling from this label
#             proportions=np.random.dirichlet(np.repeat(alpha, NUM_USERS))
...
#             for u, new_idx in enumerate(np.split(idx_l, proportions)):
#                 # add new idex to the user
#                 # if len(samples_per_user[u]) + len(new_idx) > len(max_samples_per_user):
#                 #    continue
#                 idx_batch[u] [l] = idx_batch[u].get(l, []) + new_idx.tolist()
#                 samples_per_user[u] += len(new_idx)
#                 total_sampled += len(new_idx)
#         min_size=min(samples_per_user)
#
#     ###### CREATE USER DATA SPLIT #######
#     X = [[] for _ in range(NUM_USERS)]
...
#             Labels[u].add(l)
#
#     return X, y, Labels, idx_batch, samples_per_user, total_sampled
#
# def divide_test_data(NUM_USERS, SRC_CLASSES, test_data, Labels, unknown_test):
#     # Create TEST data for each user.
#     test_X = [[] for _ in range(NUM_USERS)]
...
#     idx = {l: 0 for l in SRC_CLASSES}
#     total_sampled = 0
#     while total_sampled < len(test_data) * 0.95:
#         for user in trange(NUM_USERS):
#             if unknown_test: # use all available labels
#                 user_sampled_labels = SRC_CLASSES
#             else:
...
#     return test_X, test_y, total_sampled
# def main():
#     parser = argparse.ArgumentParser()
#     parser.add_argument("--format", "-f", type=str, default="pt", help="Format of saving: pt (torch.save), json", choices=["pt", "json"])
#     parser.add_argument("--n_class", type=int, default=10, help="number of classification labels")  # 修改为 100
#     parser.add_argument("--min_sample", type=int, default=1000, help="Min number of samples per user.")
#     parser.add_argument("--sampling_ratio", type=float, default=0.99, help="Ratio for sampling training samples.")
#     parser.add_argument("--unknown_test", type=int, default=0, help="Whether allow test label unseen for each user.")
#     parser.add_argument("--alpha", type=float, default=0.1, help="alpha in Dirichelt distribution (smaller means larger heterogeneity)")
#     parser.add_argument("--n_user", type=int, default=20,
...
#     print()
#     print("Number of users: {}".format(args.n_user))
#     print("Number of classes: {}".format(args.n_class))  # 修改为 100
#     print("Min # of samples per uesr: {}".format(args.min_sample))
#     print("Alpha for Dirichlet Distribution: {}".format(args.alpha))
#     print("Ratio for Sampling Training Data: {}".format(args.sampling_ratio))
#     NUM_USERS = args.n_user
...
#     def process_user_data(mode, data, n_sample, SRC_CLASSES, Labels=None, unknown_test=0):
#         if mode == 'train':
#             X, y, Labels, idx_batch, samples_per_user, total_sampled  = devide_train_data(
#                 data, n_sample, SRC_CLASSES, NUM_USERS, args.min_sample, args.alpha, args.sampling_ratio)
#         if mode == 'test':
#             assert Labels != None or unknown_test
#             X, y, total_sampled = divide_test_data(NUM_USERS, SRC_CLASSES, data, Labels, unknown_test)
#         dataset={'users': [], 'user_data': {}, 'num_samples': []}
#         for i in range(NUM_USERS):
#             uname='f_{0:05d}'.format(i)
#             dataset['users'].append(uname)
...
# #         return [(user_id + j) % SRC_N_CLASS for j in range(NUM_LABELS)]
# #
# # def devide_train_data(data, n_sample, SRC_CLASSES, NUM_USERS, min_sample, alpha=0.99, sampling_ratio=0.99):
# #     total_sampled = 0
# #     min_size=800
# #     ###### Determine Sampling #######
# #     while min_size < min_sample or total_sampled < n_sample * sampling_ratio :
