...
def process_and_save(config, mode="train"):
    ...
    for idx, client_csv in enumerate(client_files):
        ...
        for img_name in tqdm(client_df['img_name']):
            ...
            # 处理图片
            try:
                img = np.load(img_path)  # 使用 NumPy 加载 .npy 文件
                img = np.array(img)  # 确保是数组格式
                if img.ndim < 3:
                    img = np.stack((img,)*3, axis=-1)
                elif img.shape[2] >= 3:
                    img = img[:,:,:3]

                img = Image.fromarray(img.astype('uint8'))  # 转换为 PIL 图像
...
if __name__ == "__main__":
    config = {
        'root_dir': '.',
        'train_clients_dir': './5_clients/split_1',  # 训练集划分
        'test_clients_dir': './5_clients/split_1',  # 测试集划分
